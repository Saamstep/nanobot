...
exports.run = (client, message, args) => {
  ...
  if (client.isAdmin(message.author, message, true, client)) {
    switch (option) {
      case "youtube":
        if (args[1] === "add") {
          message.channel.send(`Adding \`${args[2]}\` to the list of youtube channel ids...`);
          return ConfigService.setConfigProperty("youtubeChannels", (ConfigService.config.youtubeChannels || []).concat(args[2]));
        } else if (args[1] === "del") {
          const ytIndex = ConfigService.config.youtubeChannels.indexOf(args[2]);

          if (ytIndex < 0) {
            return error("YouTube channel ID not found");
          }

          ConfigService.config.youtubeChannels.splice(ytIndex, 1);
          ConfigService.setConfigProperty("youtubeChannels", ConfigService.config.youtubeChannels);
          return message.channel.send(`Removed ${args[2]} from the YouTube list`);
        }

        if (args[1] !== "add" && args[1] !== "del") {
          return message.channel.send(`${ConfigService.config.prefix}set youtube [add/del] [YT channel ID]`, {
            code: "asciidoc"
          });
        }
        break;
      ...
      case "ip":
        ...
        logEvent("IP Updated", `IP has been changed to **${newIP}**`, 16776960, message, client);
        break;
      case "port":
        let newPort = args[1];
        if (newPort == null) {
          return message.channel.send(`${ConfigService.config.prefix}set port reset/[port]\nReset will make the port blank.`, {
            code: "ascidoc"
          });
        }
        if (newPort === "reset") {
          ...
          logEvent("Port Reset", `The current port has been removed`, 16776960, message, client);
        } else {
          ConfigService.setConfigProperty("mcPort", newPort);
          message.channel.send(`Port changed to \`${newPort}\``);
          logEvent("New Port", `Port has been changed to **${newPort}**`, 16776960, message, client);
        }
        break;
      case "acceptmessage":
        let tl = args[1];
        ...
        logEvent("Accept Message", `Accept Message has been changed to **${newAcceptMessage}**`, 16776960, message, client);
        break;
      case "logchannel":
        let newLog = args[1];
        if (newLog.includes("#")) {
          ...
          logEvent("Log Channel Update", `Log Channel has been changed to **${newLog}**`, 16776960, message, client);
        }
        break;
      case "servername":
        ...
        logEvent("Server Name", `Server Name has been changed to **${newServerName}**`, 16776960, message, client);
        break;
      case "debug":
        let newDebug = args[1];
        if (newDebug == "true") {
      ...
      case "website":
        ...
        if (newSite.includes("http")) {
          ...
          logEvent("Website Update", `Website has been changed to **${newLog}**`, 16776960, message);
        } else {
      ...
      case "pollchannel":
        ...
        if (newPoll.includes("#")) {
          ...
          logEvent("Poll Channel Update", `Poll channel has been changed to **${newPoll}**`, 16776960, message);
        }
        break;
      case "joinchannel":
        let chJoin = args[1];
        if (chJoin.includes("#")) {
          ...
          logEvent("Join Channel Update", `Join channel has been changed to **${chJoin}**`, 16776960, message, client);
        }
        break;
      case "game":
        let newGame = args.join(" ").slice(5);
        if (newGame == null) {
          return message.channel.send(`${ConfigService.config.prefix}set game [Game Value]`, {
            code: "asciidoc"
          });
        }
        client.user.setPresence({ game: { name: `${newGame}`, type: 0 } });
        ConfigService.setConfigProperty("defaultGame", newGame);
        message.channel.send("Updated the game status to: " + "`" + newGame + "`");
        logEvent("Default Game Update", `Default Game has been changed to **${newGame}**`, 16776960, message, client);
        break;
      case "joinmessage":
        ...
        logEvent("Join Message Update", `Join message changed to \`${newJoin}\``, 16776960, message, client);
        break;
      case "supportchannel":
        let newID = args[1];
        if (newID.includes("#")) {
          return error("Just write the numerical ID. (Do not mention the channel).", message);
        } else {
          ConfigService.setConfigProperty("supportChannelid", newID);
          message.channel.send(`Support channel  updated to <#${newID}>.`);
          logEvent("Support Channel Update", `The support channel was changed to <#${newID}>.`, 16776960, message, client);
        }
        break;
      case "nicknamechannel":
        let nickID = args[1];
        if (nickID.includes("#")) {
          return error("Just write the numerical ID. (Do not mention the channel).", message, client);
        } else {
          ConfigService.setConfigProperty("nickChannelid", nickID);
          message.channel.send(`Support channel  updated to <#${nickID}>.`);
          logEvent("Nickname Channel Update", `The nickname channel was changed to <#${nickID}>.`, 16776960, message, client);
        }
        break;
      case "leavemessage":
        ...
        logEvent("Leave Message Update", `Leave message changed to \`${newLeave}\``, 16776960, message, client);
        break;
      case "urls":
        if (args[1] == "add") {
      ...
      case "supporttags":
        ...
        if (args[1] !== "add" && args[1] !== "del") {
          return message.channel.send(`${ConfigService.config.prefix}set supporttags [add/del] [tag]`, {
            code: "asciidoc"
          });
        }
        break;
      ...
      case "streamers":
        ...
        if (args[1] !== "add" && args[1] !== "del") {
          return message.channel.send(`${ConfigService.config.prefix}set streamers [add/del] [twitch username]`, {
            code: "asciidoc"
          });
        }
      case "whitelist":
        ...
        if (args[1] !== "add" && args[1] !== "del") {
          return message.channel.send(`${ConfigService.config.prefix}set whitelist [add/del] [email]`, {
            code: "asciidoc"
          });
        }
        break;
      default:
        ...
        message.channel.send(
          `['Bot_Settings']\n\n"You can edit these values with '${ConfigService.config.prefix}set [option] [new value]'\nOptions are the names in gold with with no spaces and no caps!\nEx: ?set logchannel logger"` +
            ...
            `\nThumbs Up URLs: "${ConfigService.config.urls}"\nStreamers: "${ConfigService.config.streamers.join(", ")}'Roles'\nMod Role: "${ConfigService.config.roles.mod}"\nAdmin Role: "${ConfigService.config.roles.admin}"\nMember Role: "${

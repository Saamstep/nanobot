    ...
    switch (option) {
      case 'youtube':
        if (args[1] === 'add') {
          ...
          return ConfigService.setConfigProperty(
            'youtubeChannels',
            (ConfigService.config.youtubeChannels || []).concat(args[2])
          );
        } else if (args[1] === 'del') {
      ...
      case 'port':
        ...
        if (newPort == null) {
          return message.channel.send(
            `${ConfigService.config.prefix}set port reset/[port]\nReset will make the port blank.`,
            {
              code: 'ascidoc'
            }
          );
      ...
      case 'supporttags':
        if (args[1] == 'add') {
          ...
          return ConfigService.setConfigProperty(
            'supportTags',
            (ConfigService.config.supportTags || []).concat(args[2])
          );
        } else if (args[1] === 'del') {
